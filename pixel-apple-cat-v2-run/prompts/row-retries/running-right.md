Create Codex pet row `running-right` for `pixel-apple-cat`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Upgrade the installed Pixel Apple Cat while preserving its exact pixel-art identity, red apple body, cat face, black stem, dark outline, small hands, and feet.. Preserve silhouette, face, palette, material, proportions, markings, and props.
