Create one horizontal animation strip for Codex pet `pixel-apple-cat`, state `running-right`.
...
Identity: same pet in every frame: Upgrade the installed Pixel Apple Cat while preserving its exact pixel-art identity, red apple body, cat face, black stem, dark outline, small hands, and feet.. Preserve silhouette, face, proportions, markings, palette, material, style, and props.
Style: Pet-safe sprite: compact full-body mascot, readable in a 192x208 cell, clear silhouette, simple face, stable palette/materials, and crisp edges for chroma-key extraction. Style `pixel`: Pixel-art-adjacent digital mascot with a chunky silhouette, simple dark outline, limited palette, flat cel shading, and visible stepped edges. User style notes: Preserve the existing low-resolution pixel-art palette, hard pixel edges, proportions, and no-antialias character design..
