...
class UserAuth:
    """Small local multi-user account store for the internal-test phase."""

    def __init__(
        self,
        storage_root: Path,
        *,
        initial_username: str = "operator",
        initial_password: str = "operator123",
        session_secret: str = "",
        session_hours: int = 12,
        secure_cookie: bool = False,
        cookie_name: str = "jyd_site_session",
        create_initial: bool = True,
    ) -> None:
        self.storage_root = storage_root.resolve()
        self.storage_root.mkdir(parents=True, exist_ok=True)
        self.users_path = self.storage_root / "users.json"
        self.secret_path = self.storage_root / "user_session_secret.txt"
        self.session_seconds = max(1, int(session_hours)) * 3600
        self.secure_cookie = bool(secure_cookie)
        self.cookie_name = cookie_name
        self._lock = threading.RLock()
        self.session_secret = session_secret.strip() or self._load_or_create_secret()
        self.initial_user_created = False
        with self._lock:
            data = self._load()
            if create_initial and not data["users"]:
                username = self._validate_username(initial_username or "operator")
                password = self._validate_password(initial_password or "operator123")
                data["users"].append(self._new_record(username, password))
                self._save(data)
                self.initial_user_created = True
    ...
    def set_session_cookie(self, response: Response, user: dict[str, Any]) -> None:
        response.set_cookie(
            self.cookie_name,
    ...
    def create_user(self, username: str, password: str, *, display_name: str = "") -> dict[str, Any]:
        ...
        with self._lock:
            ...
            record = self._new_record(clean_username, clean_password, display_name=display_name)
    ...
    def update_user(
        ...
        *,
        ...
        with self._lock:
            ...
            if password:
                clean_password = self._validate_password(password)
                record.update(self._password_fields(clean_password))
    ...
    def _load(self) -> dict[str, Any]:
        if not self.users_path.is_file():
            return {"schema": "jyd.user_accounts.v1", "users": []}
        try:
            data = json.loads(self.users_path.read_text(encoding="utf-8"))
        except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise RuntimeError(f"用户账号文件损坏: {self.users_path}") from exc
        if not isinstance(data, dict) or not isinstance(data.get("users"), list):
            raise RuntimeError(f"用户账号文件格式错误: {self.users_path}")
        return data

    def _save(self, data: dict[str, Any]) -> None:
        data["schema"] = "jyd.user_accounts.v1"
