...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="从剪映采集草稿批量提取普通文字使用的花字效果。")
    parser.add_argument("--draft-dir", required=True, help="花字采集草稿目录")
    parser.add_argument(
        "--output-dir",
        default=str(WORKSPACE_ROOT / "text_effect_library"),
        help="花字素材库目录，默认使用工作区 text_effect_library",
    )
    parser.add_argument("--replace", action="store_true", help="重新复制已经收录的花字资源")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = export_text_effect_library(
            data,
            args.output_dir,
