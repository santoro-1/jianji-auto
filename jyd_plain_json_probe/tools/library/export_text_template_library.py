...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="从剪映采集草稿批量提取复合文字模板。")
    parser.add_argument("--draft-dir", required=True, help="复合文字模板采集草稿目录")
    parser.add_argument(
        "--output-dir",
        default=str(WORKSPACE_ROOT / "text_template_library"),
        help="复合文字模板素材库目录，默认使用工作区 text_template_library",
    )
    parser.add_argument("--replace", action="store_true", help="重新复制已经收录的模板资源")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        result = export_text_template_library(
            data,
            args.output_dir,
