...
from jyd_probe.sticker_export import export_sticker_library  # noqa: E402


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="从专用剪映草稿批量提取全屏贴纸。")
    parser.add_argument("--draft-dir", required=True, help="只放全屏贴纸的采集草稿目录")
    parser.add_argument(
        "--output-dir",
        default=str(WORKSPACE_ROOT / "sticker_library"),
        help="贴纸素材库目录，默认使用工作区 sticker_library",
    )
    parser.add_argument("--replace", action="store_true", help="重新复制已收录贴纸资源")
