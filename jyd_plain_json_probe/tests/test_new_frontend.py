...
class NewFrontendTest(unittest.TestCase):
    ...
    def test_new_workspace_uses_real_script_and_image_input_apis(self) -> None:
        ...
        self.assertIn("row-cover-title", html)
        self.assertIn("封面标题已独立生成，不依赖字幕或视频预览", html)
        self.assertIn("封面标题不受字幕分支失败影响", html)
    ...
    def test_variant_workspace_is_replaced_by_wrapping_h3_segment_review(self) -> None:
        ...
        self.assertIn("const isCurrentVersionPending = statusInfo.status === 'SUCCESS'", workspace)
        self.assertIn("!isActive && !isCurrentVersionPending && (canRetry || canRegenerate)", workspace)
        self.assertIn("继续下载当前版本（不收费）", workspace)
        self.assertIn("新版下载完成前不允许重新生成", workspace)
        self.assertIn("segment.local_preview_is_current === true", workspace)
