...
class NewFrontendTest(unittest.TestCase):
    ...
    def test_new_workspace_uses_real_script_and_image_input_apis(self) -> None:
        ...
        self.assertIn("const readyVideoCount = rows.filter", html)
    ...
    def test_semantic_visual_review_and_dynamic_preview_share_recipe(self) -> None:
        ...
        self.assertIn("const nameplateScale = 0.60", html)
        self.assertIn("const transformX = -0.40", html)
        self.assertIn("const transformY = -0.26", html)
    ...
    def test_complete_video_flow_keeps_internal_stages_out_of_user_results(self) -> None:
        ...
        self.assertIn("这里只更新浏览器预览，不会生成新的 MP4", workspace)
        ...
        self.assertIn("剪映将使用当前标题生成三帧封面", workspace)
        ...
        self.assertIn("const transformY = -1760 / 1920", workspace)
        self.assertIn("const fontSize = 6", workspace)
        self.assertIn("disclaimer.style.opacity = '0.5'", workspace)
