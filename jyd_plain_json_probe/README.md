...
工作台已提供 `/api/new/projects` 统一项目接口，作为新版 `/app/new` 后续分模块接入的
数据基础。一个项目包含多条脚本行，每行分别保存当前音频、当前画面合成视频、原始
视频片段、变体、字幕绑定和历史素材版本。项目、素材、异步操作和外部批次关联全部
绑定数字人账号；普通用户只能访问自己的项目。

当前完成的是公共项目骨架，不会调用 MiniMax、RunningHub 或剪映，也尚未替换新版
静态原型的模拟数据。具体模块顺序见数字人工作区根目录的 `开发顺序.md`。
