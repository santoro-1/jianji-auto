...
只保存或失效快照，不自动分析；用户点击“生成声音预览”后，工作台才对本批声音目标中首次
...
或成片，也不会触发 MiniMax、RunningHub 或剪映。

智能内容分析模块 6 已把合法 `subtitle_units` 映射到当前 MiniMax `raw_cues`：只有脚本、
分析快照、当前音频脚本摘要和 raw cues 音频绑定一致时才使用语义断点，再按抖音美好体
11 号真实字宽生成 `render_cues`。连接词、数字、专有词和禁止断开的关联单元不会在语义
路径内部拆开；空格和换行允许不出现在 MiniMax cue 文本中，`~` 必须精确匹配。任何映射
或版本校验失败都会保留 `raw_cues` 并回退旧排版。

智能内容分析模块 7 已把合法 `music_intent` 接入 46 首本地标签库并只选择唯一 Top1。
音乐分析或匹配失败时按项目默认音乐、无 BGM 降级，不阻止字幕或 4B 预览；手动曲目和
手动无 BGM 都不会被 AI 静默覆盖。当前音频变化而文本不变时复用内容分析，但会按新音频
真实时长重算选择；变体冻结继承 4B 最终配方，不把 BGM 作为变体维度。
