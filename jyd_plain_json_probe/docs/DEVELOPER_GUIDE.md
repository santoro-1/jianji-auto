...
- `device_business_transport.py` 与 `AuthCenterClient` 接通 H3 POST/GET 合同。已配置发布信任根时只向匹配源地址发送设备证明；绑定请求移除旧正文令牌、不修改输入及业务幂等键。没有可用设备凭据时可在发送前采用一次账号请求读取结果或接收云端强制拒绝，不是本地放行；发送后的拒绝不降级重发。下载证明禁止跟随重定向，也不发给 RunningHub 直达地址。
...
- 新 H3 工作目录仍属于原 `projects/<owner>/<project>/<item>/h3`，不改变账号隔离、
  `storage://` 迁移或删除项目的目录边界。其下改为平级 `s-<key>/raw.mp4`（下载）、
  `c-<key>/clean.wav|preview.mp4|report.json`（片头清理）和 `m-<key>/...`（合成）。
  key 为完整 SHA-256 的小写 Base32 编码，不截断、不按序号复用，也不受 Windows 大小写折叠影响。
- 不可变原片放在同一行 `h3/f/<完整原片 SHA-256>/segment.mp4`，不再嵌套在每个合成版本目录下；
  相同内容可复用，素材绑定仍核对完整合成签名、远端批次/行/片段和真实时长。
- 旧 `segment-cache/<key>/current.mp4` 和 `current.json` 继续读取；若当前版本已经下载，不再下载。
...
- 匹配分段台词开头最多三个连续 token，只用 ASR 实际时间戳、不用字幕插值；最多允许两个
  前导误识别 token，不匹配则本地失败。首词超过 5 秒保护上限也失败，不做固定时长降级。
- 首词之前以 10ms RMS 窗、5ms 步长找最后一个至少 20ms 的低能量区间；阈值相对首三个词
  的语音 RMS 低 25dB，并限制在一个量化单位到 -40dBFS。静音此前样本，在该区间最后 10ms
  做半余弦淡入。恢复点之后 PCM 完全不变，总样本数不变；无可用间隙则记录 `NO_HEAD_GAP`
  并保持原声。参数是当前口播样本的起始方案，不是适用于所有音频的保证。
...
- 合成缓存和素材 metadata 包含清理版本及逐段 key；旧本地结果状态同步可重新准备派生素材，
  不重做付费 H3、不覆盖已经打开或导出的用户草稿。清理未完成前不自动启动该行 4B。
...
- `h3_video_segments.py` 将可替换的 `segment-cache/current.mp4` 冻结为按 SHA-256 隔离的
  原片，登记 `original_video_segment/source_type=h3`。重生成更换引用，不改历史文件。
...
- 长路径兼容：`content_replace.py` 在所有 JSON 级素材写入后统一调用
  ...
  音频复制到输出草稿 `jyd_media/<完整 SHA-256>.<扩展名>`。副本校验后原子发布，仅改素材
