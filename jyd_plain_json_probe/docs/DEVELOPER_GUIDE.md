...
`RenderJobQueue` 提交任务。固定参数为居中、画面宽度 `0.8`、`transform_y=-856/1920`
（剪映 1080×1920 参考位置 Y=-856）、`DouyinSansBold` 14 号、默认白字、黑色 `0.06` 描边、BGM 音量 0.3。
...
时间锚点，cue 内字符时间只做确定性比例派生。`subtitle_units` 的断点进入排版时只是软偏好；
`project_postprocess.py` 先按每条 raw cue、段落和除顿号外的显式标点建立不可跨越子句，
...
- `semantic_visual_library/fixed/nameplate_zhangluo` 是每条视频自动携带的固定人名牌，不新增
  ...
  当前受控 PNG 与草稿 `jyd_eab56dad6e7e` 的启用版一致，固定参数为
  `scale=0.7331057670319187`、`transform_x=-0.26689423296808135`、
