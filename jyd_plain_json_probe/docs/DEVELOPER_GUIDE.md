...
项目成片另用两个集中参数控制收尾：`BGM_FADE_IN_US=5000000` 传入
`audios[].fade_in_us`，让最早的 BGM 片段用 5 秒渐起；`VIDEO_FADE_OUT_US=2000000` 传入
`source.fade_out_us`，只给最后一个主视频片段添加 2 秒“渐隐”。普通 4B 与变体共用这两个
