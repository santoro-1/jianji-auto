...
自动明确语义素材使用短时快切规则：默认 `duration_us=1500000`、最长 `2500000`，不同语义
素材开始时间最短间隔 `1500000` 微秒，每 60 秒最多 24 条；同 concept 或同 asset 仍至少间隔
20 秒，任何图片/视频时间区间仍不得重叠。enrichment 的 20 秒空窗规则保持不变。

正文顶部固定文字由 `texts` 自动带入单行“世界冠军带你资料”，字号 19、
