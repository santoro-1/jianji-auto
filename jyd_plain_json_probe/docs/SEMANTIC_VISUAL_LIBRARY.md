...
- 图片不得声明全屏 B-roll 或接缝 B-roll；`seam_broll` 只能是视频。历史 `loop` / `loop_allowed`
  字段决定视频能否循环；全屏或接缝目标区间长于源片时，只有 `loop_allowed=true` 才循环补足。
...
- 通用空镜由 `VISUAL_BROLL_TARGET_INTERVAL_SECONDS=15` 控制，约每 15 秒在附近寻找一个相关
  短句；实际调度至少留 8 秒空窗。每个锚点最多携带 8 个确有本地获准视频支撑的概念：先放
  当前句精确命中的视频概念，再放文章类型允许的编辑型空镜池。池名称不需要在脚本中逐字出现；
  云端必须按完整句子判断，priority 2 表示直接强相关，编辑型空镜通常使用 priority 1，没有
  自然且不误导的画面就不返回，因此 15 秒是候选节奏而不是强制配额。接缝空镜独立读取
  下一段开头语境并尝试，不占用 15 秒周期，命中后从边界起最多显示 5 秒。手工锁定、接缝、
  明确语义、通用空镜依次占位。
- 视频目标区间超过源片可用区间时，只播放从 `source_start_us` 起的剩余内容并提前结束，
  `loop_allowed=true` 时从源片起点循环补足，否则提前结束；无论是否循环，该资产仍不允许在
  同一成片的其他时间再次自动出现。
...
- 自动空镜的 `source_start_us + defaults.duration_us` 必须是完整复核过的连续干净窗口；暂停、加载、
  鼠标点击/回放图标、播放器控件不得进入该窗口。允许循环时只循环这个干净窗口，不循环整条源视频。
