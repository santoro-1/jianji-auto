...
素材描述。这五类池只参与数字人片段接缝，不参与普通 10 秒 enrichment；云端按接缝前后完整
语境判断是否存在同一具体对象、动作或明确生活场景，只有 `priority=2` 才能自动使用。仅有健康、
坚持、变好、生活、心态或相近情绪不算高相关；没有高相关素材时保留原接缝。图片不进入这些池。
...
4. 只对接缝空镜按文章类型提供五类编辑型空镜池，由模型按接缝前后完整语境选择；
...
- 通用空镜由 `VISUAL_BROLL_TARGET_INTERVAL_SECONDS=10` 控制，约每 10 秒在附近寻找一个相关
  ...
  场景概念，不再携带编辑型空镜池；`priority=1` 仅供审核，只有 `priority=2` 自动使用。只有
  ...
  不落入编辑池。接缝空镜优先读取下一段开头语境，并可结合上一段末句判断；编辑池只有命中同一
  具体对象、动作或明确生活场景且模型返回 `priority=2` 才进入成片。每个接缝最多一个素材，
