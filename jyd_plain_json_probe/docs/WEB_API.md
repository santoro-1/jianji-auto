...
- 缓存改为项目内平级 24 字符短目录，完整 key 仅保存在元数据与上述响应字段中。错误记录无法落盘时仍由进程内
...
独立处理机对 `/api/auth/center/verify` 使用 5 秒内存成功缓存；相同 token 的并发校验共享同一
次远程请求，同一 HTTP 请求中的后续项目权限判断复用中间件已经取得的用户。登录、退出和登录
接力会立即清除相关缓存和进行中引用，401 也不会保留缓存。校验使用独立的 8 秒上限；10 秒内
连续 3 次传输失败后进入 15 秒保护，期间无有效缓存的请求直接返回 503，到期只放行一个半开
探测。该保护只统计连接、DNS、TLS 和超时等传输错误，不统计 401/403 或正常业务拒绝，也不与
H3 下载熔断共用状态。高频缓存命中不逐条记录，远程校验和一分钟脱敏汇总使用
`auth_verify.*` 结构化事件，任何事件均不包含 token 或 Cookie。认证中间件通过 Starlette 受控
线程池执行阻塞式远程校验，首次缓存未命中或半开探测不会占住工作台页面事件循环。
...
`h3/s-<24字符缓存标识>/v-<24字符版本>.mp4`（兼容旧 `h3/segment-cache`、
`s-<52字符>` 和 `versions/<64位版本>`），最多并发三路；完整摘要只写入 JSON 元数据，
不再进入新目录或文件名。接口读取当前批次、当前远端 item 和当前 `segment_id` 对应的
...
项目可以同时保留多个 H3 批次。终态历史批次中只要仍有成功分段标记为
`local_preview_is_current=false`，或其 `local_download_state` 仍为排队、下载、校验、失败/失效，
`GET .../h3/status` 就必须继续查询该批次并推进本地回收，不能只同步项目级最新批次。浏览器在
当前版本落盘前不得调用付费段级重试/重生成；它只允许继续本地下载，且只有
`local_preview_is_current=true` 的成功分段才能进入原片打包。
