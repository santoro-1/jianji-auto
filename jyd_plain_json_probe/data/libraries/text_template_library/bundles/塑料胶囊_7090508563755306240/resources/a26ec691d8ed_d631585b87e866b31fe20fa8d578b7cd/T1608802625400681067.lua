...
function T1608802625400681067:seek(time)
	...
	if self.text ~= nil then
		...
		-- if time >= 0 then
		-- 	-- local str = ""
		-- 	local duration = self.duration * 0.9
		-- 	if time < duration then
		-- 		local progress = time / duration
		-- 		local blur = 20
		-- 		local value1 = bezier({0.1, 1.01, 0.9, 1.05})(progress, 0,1,1)
		-- 		local value2 = bezier({0.28, 0.7, 0.48, 0.82})(progress, 0,1,1)
		-- 		self.materials:get(0):setFloat("yoffset", value1 * 14)
		-- 		self.materials:get(0):setFloat("blurSize_all", (1-value2) * blur)
		-- 		-- self.materials:get(0):setFloat("blurSize_all", 0)
		-- 	else
		-- 		self.materials:get(0):setFloat("yoffset", 0)
		-- 		self.materials:get(0):setFloat("blurSize_all", 0)
		-- 	end
		-- end
		-- local str = ""
		...
		for i = 1, self.text.chars:size() do
			if self.text.chars:get(i-1).utf8code == "\n" then
				gap = 1
				softmask = 0.07
			end
		end	
		...
		local duration = self.duration * 0.9
		if time < duration and self.materials:get(0) then
			local progress = time / duration
			local blur = 2
			local value1 = bezier({0.1, 1.01, 0.85, 1.01})(progress, 0,1,1)
			local value2 = bezier({0.28, 0.7, 0.48, 0.82})(progress, 0,1,1)
			self.materials:get(0):setFloat("yoffset", value1 * 10 * gap)
			self.materials:get(0):setFloat("gap", gap)
			self.materials:get(0):setFloat("blurSize_all", (1-value2) * blur)
			self.materials:get(0):setFloat("progress", progress)
			self.materials:get(0):setFloat("softmask", softmask)
		else
			self.materials:get(0):setFloat("yoffset", gap)
			self.materials:get(0):setFloat("blurSize_all", 0)
			self.materials:get(0):setFloat("progress", 1)
			self.materials:get(0):setFloat("softmask", 0)
		end
		-- self.materials:get(0):setFloat("softmask", softmask)
		self.materials:get(0):setFloat("appear", appear)
	end
