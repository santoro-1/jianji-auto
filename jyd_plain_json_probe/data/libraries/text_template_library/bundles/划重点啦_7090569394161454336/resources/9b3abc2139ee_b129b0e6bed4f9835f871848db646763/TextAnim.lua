    ...
    for line, chars in pairs(self.charLineInfo) do
        ...
        for i=1, count do
            ...
            if self.text.typeSettingKind == 0 then
                ...
                gap = gap + (next_target_scale.x - ori_scale.x) * char.width * 0.55
            else
                chars[i]["cur_pos"] = Amaz.Vector3f(
                    ori_pos.x,
                    ori_pos.y - gap,
...
function TextAnim:flyIn()
    ...
    local progress = util.remap01(self.showingInfo.x, self.showingInfo.y, self.progress)
        ...
        for i=1, count do
            ...
            -- Amaz.LOGI("lrc", tostring(char.scale))
            char.position = Amaz.Vector3f(
                util.mix(
                    rect.width * 0.7, 
                    cur_pos.x, 
...
function TextAnim:seek(time)
    ...
    -- self.text.fixedRectEnabled = false
    ...
    if self.text and self.text.chars:size() > 0 then
        ...
        -- if self.text.backgroundEnabled then
        --     self.text.backgroundEnabled = false
        -- end
        ...
        if self.text.typeSettingKind == 0 then
            ...
            self.materials:get(0):setVec2("trapezoid", Amaz.Vector2f(0, self.trapezoid.w))
        end
    else
