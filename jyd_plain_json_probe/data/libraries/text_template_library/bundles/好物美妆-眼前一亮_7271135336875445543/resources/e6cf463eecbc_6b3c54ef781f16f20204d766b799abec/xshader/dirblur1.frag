...
vec4 gauss_blur(sampler2D inputTexture, vec2 uv, float angle, float blurSize, vec2 uRenderSize)
{
    ...
    #ifdef SAMPLETIIMES3
    for(int i=1; i<=SAMPLETIIMES3; i++)
    {
        curPositiveCoordinate    += dir * unit_uv;
        curNegativeCoordinate    -= dir * unit_uv;
        float fX = Gaussian(float(i)/(float(SAMPLETIIMES3)/20.0));
...
void main()
{
   ...
   vec4 color = gauss_blur(inputTexture2, uv1, angle, blurStep1, u_ScreenParams.xy);
