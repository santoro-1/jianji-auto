...
function AnimScript:seek(time)
    if isEditor then
        ...
        self.progress = self:getProgress(ANIMATIONTYPE.IN,time)
    end
    ...
    if max_line_count > 0 then
        ...
        local progress = self.util.remap(0.0,0.95,self.progress)
        ...
        for index = 1, max_line_count do
            ...
            for key, value in pairs(linneCharTab) do
                ...
                if linecount >= index then
                    ...
                    -- if key == 2 then
                    --     Amaz.LOGI("========>>",begin_scale.."dkdk"..index)
                    -- end
    ...
    for index, value in pairs(linneCharTab) do
        ...
        if linenum > 0 then
            ...
            local x1 = (ldp.x - expandSize.x)/expandSize.width
            local x2 = (rup.x - expandSize.x)/expandSize.width+0.50*avg_width

            -- if max_line_count/linenum == 1 then
            --     x2 = (rup.x - expandSize.x)/expandSize.width+1.0*avg_width
            -- end
            ...
            if self.typeSettingKind == Amaz.TypeSettingKind.VERTICAL then 
                ...
                x1 = (rup.y - expandSize.y)/expandSize.height 
            ...
            local progress = self.util.remap(0.0,0.92,self.progress*max_line_count/linenum)
            if self.typeSettingKind == Amaz.TypeSettingKind.VERTICAL then 
                progress = self.util.remap(0.0,0.88,self.progress*max_line_count/linenum)
            end
            local s_x= self.util.mix(x1,x2,progress)
            local low_blur = (x2- avg_width*2.5-x1)/(x2-x1)
            if self.typeSettingKind == Amaz.TypeSettingKind.VERTICAL then 
