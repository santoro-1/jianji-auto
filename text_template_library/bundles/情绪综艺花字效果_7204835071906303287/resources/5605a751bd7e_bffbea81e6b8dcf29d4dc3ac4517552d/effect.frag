...
in vec2 uv1;
in vec2 uv2;
in vec2 uv3; //random uv origin(minx,miny)
...
void main()
{
    ...
    tc = vec2(tex_u, tex_v);
    tc = tc * uv4 + uv3;
    diff = texture(_Diffuse, vec2(tc.x, 1.0 - tc.y))*_AnimAlpha;
...
#ifdef OUTLINE1
    cc = _Outline1Color1;
    gradDist = ((grad_u-0.5f) * _Outline1AngleVec.r + (grad_v-0.5) * _Outline1AngleVec.g)/_Outline1AngleVec.b + 0.5;
    //多层渐变色
...
#ifdef OUTLINE2
    cc = _Outline2Color1;
    gradDist = ((grad_u-0.5f) * _Outline2AngleVec.r + (grad_v-0.5) * _Outline2AngleVec.g)/_Outline2AngleVec.b + 0.5;
    //多层渐变色
...
#ifdef OUTLINE3
    cc = _Outline3Color1;
    gradDist = ((grad_u-0.5f) * _Outline3AngleVec.r + (grad_v-0.5) * _Outline3AngleVec.g)/_Outline3AngleVec.b + 0.5;
    //多层渐变色
