...
varying lowp vec2 uv1; //rect min
varying lowp vec2 uv2; //rect max
varying lowp vec2 uv3; //random uv orgin(minx,miny)
varying lowp vec2 uv4; //random scale
varying lowp vec4 chcolor;
uniform sampler2D _MainTex;
uniform sampler2D _Diffuse;
uniform vec2 _SquareSize; //用来做单字大正方形纹理的大小
...
float unpack16 (vec2 color) {
    return dot(color , bitShifts16);
}
...
void main()
{
    ...
    tc = vec2(tex_u, tex_v);
    tc = tc * uv4 + uv3;
    diff = texture2D(_Diffuse, vec2(tc.x, 1.0 - tc.y)) * _AnimAlpha;
    
    gradDist = ((grad_u-0.5) * _AngleVec.r + (grad_v-0.5) * _AngleVec.g)/_AngleVec.b + 0.5;
#ifdef BLUR
    float distance = unpack16(texture2D(_MainTex, uv0).ba);
    float inner_dis = unpack16(texture2D(_MainTex, uv0+_ShadowScale).ba);
#else
    float distance = unpack16(texture2D(_MainTex, uv0).rg);
    float inner_dis = unpack16(texture2D(_MainTex, uv0+_ShadowScale).rg);
#endif
...
#ifdef INLINE1
    cc = _Inline1Color1;
    gradDist = ((grad_u-0.5) * _Inline1AngleVec.r + (grad_v-0.5) * _Inline1AngleVec.g)/_Inline1AngleVec.b + 0.5;
    //多层渐变色
    #ifdef INLINE1COLOR2
        #ifdef INLINE1LINEAR
            x = clamp(gradDist, _Inline1ColorPercent.x, _Inline1ControlPercent.x);
            x = (x - _Inline1ColorPercent.x) / (_Inline1ControlPercent.x - _Inline1ColorPercent.x);            
            cc =  mix(cc, _Inline1Color1_2, x);
            
            x = clamp(gradDist, _Inline1ControlPercent.x, _Inline1ColorPercent.y);            
            x = (x - _Inline1ControlPercent.x) / (_Inline1ColorPercent.y - _Inline1ControlPercent.x);
            cc = mix(cc, _Inline1Color2, x);
        #else
            x = linearstep(_Inline1ControlPercent.x - 0.4 * smoothing, _Inline1ControlPercent.x + 0.4 * smoothing, gradDist);
            cc = mix(cc, _Inline1Color2, x);
        #endif // INLINE1LINEAR
                ...
                x = (x - _Inline1ColorPercent.y) / (_Inline1ControlPercent.y - _Inline1ColorPercent.y);                
                cc =  mix(cc, _Inline1Color2_3, x);
                
                x = clamp(gradDist, _Inline1ControlPercent.y, _Inline1ColorPercent.z);                
                x = (x - _Inline1ControlPercent.y) / (_Inline1ColorPercent.z - _Inline1ControlPercent.y);
                cc = mix(cc, _Inline1Color3, x);
            #else 
                x = linearstep(_Inline1ControlPercent.y - 0.4 * smoothing, _Inline1ControlPercent.y + 0.4 * smoothing, gradDist);
                cc = mix(cc, _Inline1Color3, x);
            #endif // INLINE1LINEAR
        #endif // INLINE1COLOR3
    #endif // INLINE1COLOR2
...
//------------------------------------------------
    ...
    cc = _Color1;    
        ...
        cc = mix(cc, _Color2, x);
    #else
        x = linearstep(_ControlPercent.x - 0.4 * smoothing, _ControlPercent.x + 0.4 * smoothing, gradDist);
        cc = mix(cc, _Color2, x);
    #endif // LINEAR
...
#ifdef OUTLINE1
    cc = _Outline1Color1;
    gradDist = ((grad_u-0.5) * _Outline1AngleVec.r + (grad_v-0.5) * _Outline1AngleVec.g)/_Outline1AngleVec.b + 0.5;
    //多层渐变色
    #ifdef OUTLINE1COLOR2
        #ifdef OUTLINE1LINEAR
            x = clamp(gradDist, _Outline1ColorPercent.x, _Outline1ControlPercent.x);
            x = (x - _Outline1ColorPercent.x) / (_Outline1ControlPercent.x - _Outline1ColorPercent.x);            
            cc =  mix(cc, _Outline1Color1_2, x);
            
            x = clamp(gradDist, _Outline1ControlPercent.x, _Outline1ColorPercent.y);            
            x = (x - _Outline1ControlPercent.x) / (_Outline1ColorPercent.y - _Outline1ControlPercent.x);
            cc = mix(cc, _Outline1Color2, x);
        #else
            x = linearstep(_Outline1ControlPercent.x - 0.4 * smoothing, _Outline1ControlPercent.x + 0.4 * smoothing, gradDist);
            cc = mix(cc, _Outline1Color2, x);
                ...
                x = (x - _Outline1ColorPercent.y) / (_Outline1ControlPercent.y - _Outline1ColorPercent.y);                
                cc =  mix(cc, _Outline1Color2_3, x);
                
                x = clamp(gradDist, _Outline1ControlPercent.y, _Outline1ColorPercent.z);                
                x = (x - _Outline1ControlPercent.y) / (_Outline1ColorPercent.z - _Outline1ControlPercent.y);
                cc = mix(cc, _Outline1Color3, x);
            #else 
                x = linearstep(_Outline1ControlPercent.y - 0.4 * smoothing, _Outline1ControlPercent.y + 0.4 * smoothing, gradDist);
                cc = mix(cc, _Outline1Color3, x);
            #endif // OUTLINE1LINEAR
    ...
    FragColor = mix(cc, FragColor, alpha);
    //计算好下一次alpha值
    l = clamp(1.0 - _Outline1Scale - smoothing, 0.0, 1.0);
    alpha = linearstep(l, 1.0 - _Outline1Scale + smoothing, distance);

#ifdef OUTLINE2
    cc = _Outline2Color1;
    gradDist = ((grad_u-0.5) * _Outline2AngleVec.r + (grad_v-0.5) * _Outline2AngleVec.g)/_Outline2AngleVec.b + 0.5;
    //多层渐变色
    #ifdef OUTLINE2COLOR2
        #ifdef OUTLINE2LINEAR
            x = clamp(gradDist, _Outline2ColorPercent.x, _Outline2ControlPercent.x);
            x = (x - _Outline2ColorPercent.x) / (_Outline2ControlPercent.x - _Outline2ColorPercent.x);            
            cc =  mix(cc, _Outline2Color1_2, x);
            
            x = clamp(gradDist, _Outline2ControlPercent.x, _Outline2ColorPercent.y);            
            x = (x - _Outline2ControlPercent.x) / (_Outline2ColorPercent.y - _Outline2ControlPercent.x);
            cc = mix(cc, _Outline2Color2, x);
        #else
            x = linearstep(_Outline2ControlPercent.x - 0.4 * smoothing, _Outline2ControlPercent.x + 0.4 * smoothing, gradDist);
            cc = mix(cc, _Outline2Color2, x);
                ...
                x = (x - _Outline2ColorPercent.y) / (_Outline2ControlPercent.y - _Outline2ColorPercent.y);                
                cc =  mix(cc, _Outline2Color2_3, x);
                
                x = clamp(gradDist, _Outline2ControlPercent.y, _Outline2ColorPercent.z);                
                x = (x - _Outline2ControlPercent.y) / (_Outline2ColorPercent.z - _Outline2ControlPercent.y);
                cc = mix(cc, _Outline2Color3, x);
            #else 
                x = linearstep(_Outline2ControlPercent.y - 0.4 * smoothing, _Outline2ControlPercent.y + 0.4 * smoothing, gradDist);
                cc = mix(cc, _Outline2Color3, x);
            #endif // OUTLINE2LINEAR
    ...
    FragColor = mix(cc, FragColor, alpha);
    //计算好下一次alpha值
    l = clamp(1.0 - _Outline2Scale - smoothing, 0.0, 1.0);
    alpha = linearstep(l, 1.0 - _Outline2Scale + smoothing, distance);

#ifdef OUTLINE3
    cc = _Outline3Color1;
    gradDist = ((grad_u-0.5) * _Outline3AngleVec.r + (grad_v-0.5) * _Outline3AngleVec.g)/_Outline3AngleVec.b + 0.5;
    //多层渐变色
    #ifdef OUTLINE3COLOR2
        #ifdef OUTLINE3LINEAR
            x = clamp(gradDist, _Outline3ColorPercent.x, _Outline3ControlPercent.x);
            x = (x - _Outline3ColorPercent.x) / (_Outline3ControlPercent.x - _Outline3ColorPercent.x);            
            cc =  mix(cc, _Outline3Color1_2, x);
            
            x = clamp(gradDist, _Outline3ControlPercent.x, _Outline3ColorPercent.y);            
            x = (x - _Outline3ControlPercent.x) / (_Outline3ColorPercent.y - _Outline3ControlPercent.x);
            cc = mix(cc, _Outline3Color2, x);
        #else
            x = linearstep(_Outline3ControlPercent.x - 0.4 * smoothing, _Outline3ControlPercent.x + 0.4 * smoothing, gradDist);
            cc = mix(cc, _Outline3Color2, x);
                ...
                x = (x - _Outline3ColorPercent.y) / (_Outline3ControlPercent.y - _Outline3ColorPercent.y);                
                cc =  mix(cc, _Outline3Color2_3, x);
                
                x = clamp(gradDist, _Outline3ControlPercent.y, _Outline3ColorPercent.z);                
                x = (x - _Outline3ControlPercent.y) / (_Outline3ColorPercent.z - _Outline3ControlPercent.y);
                cc = mix(cc, _Outline3Color3, x);
            #else 
                x = linearstep(_Outline3ControlPercent.y - 0.4 * smoothing, _Outline3ControlPercent.y + 0.4 * smoothing, gradDist);
                cc = mix(cc, _Outline3Color3, x);
            #endif // OUTLINE3LINEAR
    ...
    FragColor = mix(cc, FragColor, alpha);
    //计算好下一次alpha值
    l = clamp(1.0 - _Outline3Scale - smoothing, 0.0, 1.0);
    alpha = linearstep(l, 1.0 - _Outline3Scale + smoothing, distance);
#endif // OUTLINE3
#endif // OUTLINE2
#endif // OUTLINE1
...
#ifdef INNERSHADOW
    //额外裁剪掉掉出字本体的部分
    float inner_alpha = linearstep(_Scale-smoothing_init,_Scale+smoothing_init, inner_dis);
    alpha*=inner_alpha;
#endif
