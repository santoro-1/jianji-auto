...
function util.clamp(_min, _max, value)
...
function util.valueNoise(seed, freq)
    local id = Amaz.Vector2f(math.floor(seed.x * freq), math.floor(seed.y))
    local id1 = Amaz.Vector2f(math.floor(seed.x * freq) + 1.0, math.floor(seed.y))
    local id2 = Amaz.Vector2f(math.floor(seed.x * freq), math.floor(seed.y) + 1.0)
    local id3 = Amaz.Vector2f(math.floor(seed.x * freq) + 1.0, math.floor(seed.y) + 1.0)
    local fdx = smoothstep(0.0, 1.0, (seed.x * freq) % 1.0)
    local fdy = smoothstep(0.0, 1.0, (seed.y) % 1.0)
    fdx = (seed.x * freq) % 1.0
    fdy = (seed.y) % 1.0
    local n1 = random(id)
    local n2 = random(id1)
...
function util.bezierWithParams(input_val_4, min_val, max_val, in_val, reverse)
    if type(input_val_4) == "tabke" then
        if reverse == nil then
            return util.bezier(input_val_4)(util.remap(min_val, max_val, in_val), 0, 1, 1)
        else
            return util.bezier(input_val_4)(1-util.remap(min_val, max_val, in_val), 0, 1, 1)
        end
    else
        if reverse == nil then
            return util.bezier(util.changeVec4ToTable(input_val_4))(util.remap(min_val, max_val, in_val), 0, 1, 1)
        else
            return util.bezier(util.changeVec4ToTable(input_val_4))(1-util.remap(min_val, max_val, in_val), 0, 1, 1)
        end
    end
end
