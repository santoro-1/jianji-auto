...
local BounceOut = BounceOut or {}
BounceOut.__index = BounceOut
function BounceOut.new(construct, ...)
    local self = setmetatable({}, BounceOut)
    self.duration = 1.0
    self.count = 0
    if construct and BounceOut.constructor then BounceOut.constructor(self, ...) end
    return self
end

function BounceOut:constructor()

end

function BounceOut:onStart(comp) 
	self.text = comp.entity:getComponent("SDFText")
	self.trans = comp.entity:getComponent("Transform")
	-- self.text.str = 'BounceOut'
...
-- local time = 0
-- function BounceOut:onUpdate(comp, deltaTime)
-- 	time = time + deltaTime
...
function BounceOut:setDuration(duration)
   self.duration = duration
end

function BounceOut:clear()
	if self.text ~= nil then
