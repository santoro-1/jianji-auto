...
void main()
{
    float theta = 0.0+u_Angle * 3.1415926;
    vec2 dir =  gauss_stron1*vec2(cos(theta), sin(theta)) / vec2(720.,720.*u_ScreenParams.y/u_ScreenParams.x);
    vec4 color = gaussianBlur(inputTexture5, uv0, dir, u_Strength);
    gl_FragColor = color;
}
